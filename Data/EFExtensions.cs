...
namespace ArmsFW.Data.Extensions
{
    public static class EFDataExtensions
	{
		...
		/// <summary>
		/// Data uma query de entidades, aplica regras de pagina��o e retorna em um objeto Pesquisa
		/// </summary>
		/// <typeparam name="T">Tipo de retorno</typeparam>
		/// <param name="query">Instancia da query</param>
		/// <param name="paginacao">Defini��es de pagina��o</param>
		/// <returns></returns>
		public static Pesquisa<T> ToListaPaginada<T>(this IQueryable<T> query, PaginacaoRequest paginacao)
		{
			...
			try
            {
				...
				//Se informado as regras de pagina��, aplica
				if (paginacao != null)
				{
				...
				retorno = ResultBase.Sucesso("Consulta conclu�da com sucesso no DB");
			}
            catch (Exception ex)
            {
				ex.Logar();
